...
class LLMobserver(yarp.RFModule):
    ...
    def configure(self, rf) :
        ...
        self.character = 'You are the observer of the environment and provide feedback to another LLM that is in control of a humanoid robot called ErgoCub, designed to collaborate with humans in physical cooperative tasks, like preparing a salad.\n\
                    ...
                    In the [ENVIRONMENT] section you describe ONLY the environment, in the [PEOPLE] part you describe if there is a person and how s-/he looks like and in [OBJECTS] you list the objects you see.\n\
                    If nothing has changed in the environment with respect to the previous description that you read, return [ENVIRONMENT]: No changes.\n\
                    If the person or the number of people doesnt really change  with respect to the previous description that you read, return [PEOPLE]: No changes.\n\
                    If the objects or the number of available tools doesn/t really change with respect to the previous description that you read, please return [OBJECTS]: No changes.\n\
                    Remember to always include the fields [ENVIRONMENT], [PEOPLE], and [OBJECTS] in your response and report the info in the specific field, that is no info on objects or people in the [ENVIRONMENT] field\n'
    ...
    def updateModule(self):
        ...
        response = generate('llava', 'Which objects do you see?', images=[base64_image], stream=False)
